...
use crate::{
    ...
    io::midi::midi_instrument::{MidiInstrument, MidiInstrumentSet},
    noteblock::Note,
};
...
pub struct Midi2NbsDecoder(MidiInstrumentSet);

impl Midi2NbsDecoder {
    pub fn new() -> Self {
        Midi2NbsDecoder(MidiInstrumentSet::default())
    }

    pub fn with_midi_instrument(instrument_set: MidiInstrumentSet) -> Self {
        Midi2NbsDecoder(instrument_set)
    }

    pub fn decode(&self, midi_bytes: &[u8]) -> Result<Nbs, MidiDecodeError> {
                    ...
                    TrackEventKind::Midi { channel, message } => match message {
                        MidiMessage::NoteOn { key: note, vel } => {
                            ...
                            let note_entry = if channel == 9 {
                                if let Some(drum) = self.0.get_drum(note.as_int() as usize) {
